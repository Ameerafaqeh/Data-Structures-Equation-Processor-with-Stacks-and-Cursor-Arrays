...
public class Te {
	...
	public static String processSection(String section) {
		...
		for (String line : lines) {
			...
			if ((isInInfixSection || isInPostfixSection) && line.startsWith("<equation>")) {
				results.append(processEquation(line, isInInfixSection)).append("\n");
				operationsProcessed++;

				if (operationsProcessed == 11) {
					// Stop processing after the first 7 operations
					break;
				}
			}
		}
	...
	private static String processEquation(String equation, boolean isInInfix) {
		StringBuilder resultStringBuilder = new StringBuilder();

		resultStringBuilder.append("Processing ").append(isInInfix ? "Infix" : "Postfix").append(" Equation: ")
				.append(equation).append("\n");

		try {
			String result = isInInfix ? infixToPostfix(equation) : postfixToPrefix(equation);
			resultStringBuilder.append(isInInfix ? "Postfix" : "Prefix").append(": ").append(result).append("\n");
			resultStringBuilder.append("Result: ");
			resultStringBuilder.append(evaluateExpression(result, isInInfix));
		} catch (IllegalArgumentException e) {
			resultStringBuilder.append("Error: ").append(e.getMessage());
		}
	...
	private static String evaluateExpression(String expression, boolean isInInfix) {
		try {
			double result;
			if (isInInfix) {
